# 0011 — Conduit Workstream 1: known gaps and forward notes

Date:        2026-08-20
Status:      accepted
Decided by:  Angel

Context:     Conduit's Workstream 1 (EDGAR ingestion, 137 tests, live EDGAR verified) is COMPLETE.
...
**1. Ingestion verified against one clean filer only.**
   ...
   notification processing. Before Ledger's ingestion is considered proven in the field, a second
...
**2. Canvas rendering is untestable in jsdom; Forge will need Playwright.**
   Vitest's jsdom environment cannot test Canvas rendering. Forge's render-correctness work
   requires browser mode and Playwright, which is an unapproved future dependency escalation (not
   covered by 0008's dependency approval). When Forge needs to test rendering, this requires an
   Angel approval before Playwright is added.
   **Blocks nothing.** The slice uses only Forge's geometry algorithms; rendering is Cartographer's
   next phase.

**3. Rate limiter is per-process; clustered Node needs escalation.**
   Conduit implements the 10 request/second limit as an in-memory counter per process. A second
...
**4. filerCategory is newly surfaced and unblocks Invariant 2.5 late-filing classification.**
   ...
   (in the submissions document field `category`), and Conduit now surfaces it as part of
   `CompanySubmissions.filerCategory`. The comment in filings.ts has been corrected, and Invariant
   2.5's classification rule is no longer blocked by data availability. **Ledger must be told
   explicitly** — this was a standing reason for a deferred design question, and the deferral is
   now lifted.
   **Unblocks Ledger's Invariant 2.5 work.** Ledger can now distinguish late filings once it
...
**5. 30fps performance floor is assumed, not measured.**
   ...
   real geometries), so measurement was deliberately deferred. When Forge's rendering is complete,
   Forge and Angel will measure actual frame rates against the 30 fps assumption.
...
**6. No visual encoding has been observed.**
   ...
   No human has yet opened the application and seen it render. Cartographer's lake encoding is
...
Consequence: Conduit's work is COMPLETE. Workstream 2 (segment extraction, Ledger) is now
             unblocked, though it must start with a sync conversation with Cartographer to agree
             ...
             Ledger should confirm that filerCategory is what it needs for late-filing
             classification; if so, the feature becomes available immediately once Ledger's
