...
test.describe('Invariant 4.1 — reference load', () => {
  test('capped 12-segment reference load holds the locked rate', async ({ page }, testInfo) => {
    ...
    expect(budget.pacing.p99Ms, failedChecks(budget)).toBeLessThanOrEqual(
      budget.lockedIntervalMs * 1.5,
    );
    expect(budget.pacing.worstMs).toBeLessThanOrEqual(budget.lockedIntervalMs * 1.5);
  });
...
test.describe('Regression gate', () => {
  test('does not regress against the recorded baseline', async ({ page }, testInfo) => {
    ...
    for (let i = 0; i < 30; i += 1) await page.mouse.move(140 + i * 40, 260 + (i % 5) * 60);
